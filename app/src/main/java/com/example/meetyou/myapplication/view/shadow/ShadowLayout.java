package com.example.meetyou.myapplication.view.shadow;
...
import com.example.meetyou.myapplication.utils.DeviceUtils;
...
public class ShadowLayout extends LinearLayout {
    ...
    //默认阴影半径
    public static float SHADOW_DEFAULT_RADIUS = 0;

    //阴影最大偏移量
    public static float SHADOW_MAX_OFFSET = 0;

    //阴影最大模糊半径
    public static float SHADOW_MAX_BLUR = 0;



    //默认模糊半径
    public static float SHADOW_DEFAULT_BLUR_RADIUS = 0;
    ...
    //阴影类型
    private int shadowType = Right | Bottom;
    ...
    public ShadowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        SHADOW_DEFAULT_RADIUS =DeviceUtils.dip2px(context, 16);
        //阴影最大偏移量
        SHADOW_MAX_OFFSET = DeviceUtils.dip2px(context, 20);
        //阴影最大模糊半径
        SHADOW_MAX_BLUR = DeviceUtils.dip2px(context, 20);
        //默认模糊半径
        SHADOW_DEFAULT_BLUR_RADIUS = DeviceUtils.dip2px(context, 5);
        xOffset  = DeviceUtils.dip2px(context, 10);
        yOffset = DeviceUtils.dip2px(context, 10);
        ...
        shadowColor = typedArray.getColor(R.styleable.ShadowLayout_shadowColor, Color.GRAY);
        ...
        shadowType =  typedArray.getInt(R.styleable.ShadowLayout_shadowType,Right|Bottom);
        hasEffect = typedArray.getBoolean(R.styleable.ShadowLayout_hasEffect, false);
        xOffset = typedArray.getDimension(R.styleable.ShadowLayout_xOffset, DeviceUtils.dip2px(context, 10));
        yOffset = typedArray.getDimension(R.styleable.ShadowLayout_yOffset, DeviceUtils.dip2px(context, 10));
    ...
    private void init(){
        if (xOffset>0){
            ...
            if ((shadowType & Left) == Left ) {
                left = (int) blurRadius;
            }
            if ((shadowType & Right) == Right) {
        ...
        if (yOffset>0){
            ...
            if ((shadowType & Top) == Top ) {
                //水平偏移为0,水平间距为blurRadius
                top = (int) blurRadius;
            }
            if ((shadowType & Bottom) == Bottom) {
