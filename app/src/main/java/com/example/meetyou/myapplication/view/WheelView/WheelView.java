package com.example.meetyou.myapplication.view.WheelView;
...
public class WheelView extends View {
    ...
    private void setCurrentItem(int index, boolean animated) {
        ...
        if (index >= 0 && index < adapter.length) {
            isValueChanged = !TextUtils.equals(adapter[index], lastValue);
        }
        if (index != currentItem || isValueChanged) {
            if (animated) {
                ...
                int old = currentItem;
                currentItem = index;
                if (currentItem >= 0 && currentItem < adapter.length) {
                    lastValue = adapter[currentItem];
                }
