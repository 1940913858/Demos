...
import com.example.meetyou.myapplication.DemoApplication;
import com.example.meetyou.myapplication.R;
import com.example.meetyou.myapplication.video.utils.HttpFileListener;
import com.example.meetyou.myapplication.video.utils.HttpFileUtil;
import com.example.meetyou.myapplication.video.utils.VideoDeviceUtil;
import com.example.meetyou.myapplication.video.utils.VideoUtil;
...
public class VideoMaterialDownloadProgress {
    ...
    public void start(Downloadlistener listener) {
        ...
        HttpFileListener fileListener = new HttpFileListener() {
            ...
            public void onSaveSuccess(File file) {
                ...
                if (TextUtils.isEmpty(dataDir)) {
                    mListener.onDownloadFail(DemoApplication.getApplication().getString(R.string.video_material_download_progress_material_unzip_failed));
                    stop();
                    return;
                }
                file.delete();
            ...
            public void onSaveFailed(File file, Exception e) {
                mListener.onDownloadFail(DemoApplication.getApplication().getString(R.string.video_material_download_progress_download_failed));
                stop();
            }
        ...
        if (onlineMaterialDir == null || onlineMaterialDir.getName().startsWith("null")) {
            mListener.onDownloadFail(DemoApplication.getApplication().getString(R.string.video_material_download_progress_no_enough_storage_space));
            stop();
            return;
        }
